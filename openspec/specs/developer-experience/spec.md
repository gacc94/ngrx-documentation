...
Define the developer experience toolchain for code quality, formatting, linting, commit standards, and changelog generation in the ngrx-documentation project.
...
the system SHALL validate it follows the Conventional Commits format with emoji prefixes.
...
the system SHALL use a `biome.json` file that editors can discover automatically.

#### Scenario: VS Code Integration
GIVEN a developer opens the project in VS Code with the Biome extension
WHEN the editor detects `biome.json` in the project root
...
GIVEN a developer runs `bun biome check` in the terminal
WHEN Biome reads the `biome.json` configuration
...
the system SHALL run Biome checks, TypeScript type checking, and tests via GitHub Actions.

#### Scenario: CI Quality Check
GIVEN a developer pushes to `main` or opens a PR
WHEN GitHub Actions triggers the CI workflow
THEN Biome checks format and lint
AND TypeScript type checking passes
AND the pipeline fails if any checks fail

#### Scenario: CI Test Execution
GIVEN the quality checks pass
...
WHEN a new release is made via `bun version`,
the system SHALL generate and stage a CHANGELOG.md based on conventional commit history.

#### Scenario: Changelog on Version Bump
GIVEN the `preversion` script is configured
WHEN `npm version patch|minor|major` is run
THEN conventional-changelog generates CHANGELOG.md from commit history
AND the changelog is staged for the release commit

#### Scenario: Manual Changelog Generation
GIVEN a developer wants to preview the changelog
WHEN the developer runs `bun run changelog`
THEN conventional-changelog generates CHANGELOG.md without modifying package.json
